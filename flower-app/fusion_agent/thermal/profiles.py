...
def density_profile(
    r: np.ndarray, n_axis: float, a_minor: float, peaking: float = 0.8
) -> np.ndarray:
...
def solve_temperature(r, n_e, source, chi_gb, T_edge_J, R_major):
    ...
    for i in range(len(r) - 2, -1, -1):
        ...
        def flux(T_in, T_out=T_out, step=step, n_mid=n_mid):
            ...
            chi = max(
                float(chi_of_gradient(chi_gb, np.array(R_major * abs(grad) / T_mid))),
                1e-12,
            )
