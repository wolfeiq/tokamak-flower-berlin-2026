...
from fusion_agent.thermal import profiles as P
from fusion_agent.thermal._closure import chi_from_power_balance

DEVICES = json.loads((Path(P.__file__).with_name("_devices.json")).read_text())
WINDOW = slice(10, 90)
...
def test_solution_conserves_flux(site, fraction):
    ...
    q_mid = 0.5 * (q[:-1] + q[1:])
    n_mid = 0.5 * (n[:-1] + n[1:])
    residual = np.abs(n_mid * chi[:-1] * grad - q_mid) / np.maximum(
        np.abs(q_mid), 1e-30
    )
...
def test_weakly_heated_profile_stays_below_the_critical_gradient():
    ...
    r_over_lt = (
        dev["R_major"]
        * np.abs(np.gradient(c["T_e"], c["r"]))
        / np.maximum(c["T_e"], 1e-30)
    )
...
def test_ground_truth_never_reaches_a_released_product():
    from fusion_agent.thermal.core import analyse_site
